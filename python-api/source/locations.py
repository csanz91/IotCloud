...
class UserLocations():
    ...
    def on_post(self, req, resp, userId):
        ...
        try:
            locationId = dbinterface.insertLocation(self.db, 
                                                    userId,
...
class LocationsPermissions():
    ...
    def on_post(self, req, resp, userId, locationId):
        ...
        try:
            ...
            if not otherUserId:
                ...
                raise ValueError("The permissions are granted to another user, please use other user's email")
            ...
            result = dbinterface.insertUserLocationShare(self.db, otherUserId, userId, locationId, req.media['email'], req.media['role'])
            
        except ValueError as e:
            logger.error("The userId: %s has requested to give location permissions to the following email: %s that is not valid" % (userId, req.media['email']))
            raise falcon.HTTPBadRequest(
    ...
    def on_get(self, req, resp, userId, locationId):
        ...
        try:
            locationPermissions = dbinterface.selectLocationShares(self.db, userId, locationId)
        except:
            logger.error("Exception. userId: %s, locationId: %s" % (userId, locationId), exc_info=True)
            raise falcon.HTTPBadRequest(
                'Bad Request',
