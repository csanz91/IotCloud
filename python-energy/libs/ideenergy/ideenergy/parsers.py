...
def parse_historical_consumption_pd(data) -> pd.DataFrame:

    start = tz.localize(
        datetime.strptime(data[0]["fechaDesde"], "%d-%m-%Y").replace(
    ...
    values = [
        [
            (start + timedelta(hours=idx)).astimezone(tz),
            data[0]["cups"],
            value,
            data[0]["valoresPeriodosTarifarios"][idx][0],
            data[0]["valoresPeriodosTarifarios"][idx][1],
            data[0]["valoresPeriodosTarifarios"][idx][2],
        ]
        for idx, value in enumerate(data[0]["valores"])
    ]

    consumption = pd.DataFrame(columns=columns, data=values)
    ...
    consumption[columns_to_divide] = consumption[columns_to_divide] / 1000.0
    consumption[columns_to_divide] = consumption[columns_to_divide].astype(float)
    consumption.fillna(0.0, inplace=True)
